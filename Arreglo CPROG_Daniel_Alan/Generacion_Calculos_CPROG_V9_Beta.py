...
pd_1 = pd.read_excel('Cargos CPROG_V3.xlsx', sheet_name = 'FechaIniVig', nrows= 50, usecols = 'A:D') #Ojo Esta limitado a 50 OR
pd_1 = pd_1.dropna()
...
result_2 = pd.merge(result, DF_Agentes, left_on='AOM', right_on='IDENTAGENT',how = 'left')
...
wb = load_workbook('Cargos CPROG_V3.xlsx')
ws = wb['FechaIniVig']
...
uti.Copiar_hoja_Excel('FechaIniVig', 'FechaIniVig2', 'Cargos CPROG_V3.xlsx', wb)
wb = load_workbook('Cargos CPROG_V3.xlsx')
...
raiz_guide.geometry("350x380") #Definir el tamaño de la ventana
...
User_Ocle_label.place(x=60,y=210)
User_Oracle = Entry(raiz_guide)
User_Oracle.place(x=60, y=230)


Passw_Ocle_label=Label(text="Contraseña Oracle",font=("Arial",10)) 
Passw_Ocle_label.place(x=60,y=250)
Passw_Oracle = Entry(raiz_guide, show="*")
Passw_Oracle.place(x=60, y=270)
...
for i in range(len(Agentes)):
    ...
    fecha_ini_age = str(result_3[result_3['AOM']==Agentes[i]]['Fecha Ini Vig'].unique()[0])
...
DF_027 = get027Index(Mes,BUser_Oracle,BPassw_Oracle)
...
for Age in Agentes:
    ...
    if ((Age == 'MARM') or (Age == 'SOLM')):
        Fehca_m_o = '2019-12-01'
        uti.Copiar_Valor_Variable_Excel('IPPo_string', wb,'IPPo (Diciembre de 2019)')
    else:
        Fehca_m_o = '2017-12-01'
    Fecha_ipp_a = result_3[result_3['AOM']==Age]['Fecha IPPaa'].unique()[0]
    ...
    ## <-------------------------- Agregar el codigo del Factor
    try:
        uti.Copiar_Valor_Variable_Excel('Factor', wb,DF_027[DF_027['Market']==Age]['Factor'].unique()[0])
        # print((DF_027.loc[DF_027['Market'] == Age]['Factor'].unique()[0]))
    except:
        factorNo027 =  DF_Ippa[DF_Ippa['Fecha']==Fecha_m_2]['IPP'].unique()[0] / DF_Ippa[DF_Ippa['Fecha']==Fehca_m_o]['IPP'].unique()[0]
        uti.Copiar_Valor_Variable_Excel('Factor', wb, factorNo027)
        
    if ((Age == 'MARM') or (Age == 'SOLM')):
        uti.Copiar_Valor_Variable_Excel('IPPo_string', wb,'IPPo (Diciembre de 2019)')
    else:
        uti.Copiar_Valor_Variable_Excel('IPPo_string', wb,'IPPo (Diciembre de 2017)')
    ...
    Fecha_ini_RemunPGP = result_3[result_3['AOM']==Age]['Fecha Ini Vig'].iloc[0]
    ...
    uti.Copiar_hoja_Excel('CargoCPROG_1', 'CargoCPROG-'+str(Age), 'Cargos CPROG_V3.xlsx',wb)
    uti.Limpiar_plantilla(ws)
    
uti.Limpiar_plantilla(ws)
ws.sheet_view.showGridLines = False
wb.save(directorio+'Cargos CPROG_'+str(Mes_str)+"_"+str(tipo_user)+'_V9.xlsx')
wb.save('Cargos CPROG_V3.xlsx')
