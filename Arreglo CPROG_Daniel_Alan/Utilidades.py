...
def Consulta_arg_1(cnxn, nombre_query, vable_1, arg_1):
    fd = open('Querys_CPROG\\'+nombre_query,'r', encoding='cp1252').read()
...
def Consulta_Insumos_AIMCP(result_3, Age):
    cnxn = ConexionMID()
    # cnxn = ConexionMIDSDL()
    NMA = result_3[result_3['AOM']==Age]['NMA'].unique()[0]
    Ung_query= "'" + str(result_3[result_3['AOM']==Age]['UngID'].unique()[0]) + "'"
...
def Limpiar_plantilla(ws):
    ...
    Limpiar_variables(ws,24,100,2,8)
    Limpiar_variables(ws,23,24,2,7,thin_border)
    Limpiar_variables(ws,4,8,8,11,thin_border)
    for i in range(7, 8):
