...
import numpy as np
import sys
from Generacion_Calculos_CPROG_V9_Beta import DF_027
...
def verificacion_CPROG(Mes,tipo_user, mapeo_meses, Df_Agentes, directorio, directorio_archivos):
    ...
    Archivo_verif =  'Cargos CPROG_'+Mes+"_"+tipo_user+"_V9.xlsx"
...
#    wb.save(directorio+Archivo_verif_sin_form)
    ...
    #Insumos para acceder a archivos de publicación y verificación
        ...
        #comento
        try:
            if Age in DF_027['Market']:
                df_AIMCP = pd.read_excel(directorio_archivos+Archivo, sheet_name = 'CargoCPROG', nrows= 36, usecols = 'B:G', skiprows= range(1,35), header = None)
                df_AIMCP.columns = ['MES','IPPM2','VenSTN','VenNI','VenI','CPROGAIM']
                df_AIMCP.dropna(axis = 0, inplace = True)
                
                df_insumos_CPROG = pd.read_excel(directorio_archivos+Archivo, sheet_name = 'CargoCPROG', nrows= 2, usecols = 'B:H', skiprows= range(1,28), header = None)
                ...
                df_IPPs = pd.read_excel(directorio_archivos+Archivo, sheet_name = 'CargoCPROG', nrows= 4, usecols = 'B:C', skiprows= range(1,27), header = None)
                df_IPPs.columns = ['Indicador','Valor']
                df_IPPs.dropna(axis = 0, inplace = True)
                
            else:
                df_AIMCP = pd.read_excel(directorio_archivos+Archivo, sheet_name = 'CargoCPROG', nrows= 36, usecols = 'B:G', skiprows= range(1,24), header = None)
                df_AIMCP.columns = ['MES','IPPM2','VenSTN','VenNI','VenI','CPROGAIM']
                df_AIMCP.dropna(axis = 0, inplace = True)
            ...
            df_AIMCP_verif = pd.read_excel(directorio+Archivo_verif_sin_form, sheet_name = Nombre_hoja, nrows= 36, usecols = 'B:G', skiprows= range(1,22), header = None)
